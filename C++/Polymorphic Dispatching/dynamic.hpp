...
template<size_type p0, typename Filler = void>
struct BaseAdverb { };

template<typename Filler>
struct BaseAdverb<1, Filler> { int count; };

//

template<typename value_type>
value_type square(const BaseAdverb<0> & a, value_type x)
{
	return x*x;
}

template<typename value_type>
value_type square(BaseAdverb<1> & a, value_type x)
{
	++a.count;
...
struct Adverb : public BaseAdverb<0> { };

template<typename Filler>
struct Adverb<0, 0, 0, 0, Filler> : BaseAdverb<1> { };
